...
$(document).ready(function () {
	...
	function updateDraftBadgeCount(drafts, draftsEl) {
	...
	function renderEmptyDraftState(draftListEl) {
	...
	function escapeDraftItem(draft) {
		if (!draft) {
			return;
		}
		if (draft.title) {
	...
	async function renderDraftList(drafts) {
		const draftListEl = $('[component="drafts/list"]');
		const draftItems = drafts.listAvailable();
		if (!draftItems.length) {
			renderEmptyDraftState(draftListEl);
			return;
		}

		draftItems.reverse().forEach(escapeDraftItem);
	...
	function bindDraftHandlers(draftsEl, drafts, bootbox) {
		const renderList = function () {
			return renderDraftList(drafts);
		};

		draftsEl.on('shown.bs.dropdown', renderList);
	...
	function initializeDraftsSidebar(drafts, bootbox) {
		const draftsEl = $('[component="sidebar/drafts"]');
		bindDraftHandlers(draftsEl, drafts, bootbox);

		const updateCount = function () {
			updateDraftBadgeCount(drafts, draftsEl);
		};

		$(window).on('action:composer.drafts.save', updateCount);
		$(window).on('action:composer.drafts.remove', updateCount);
		updateCount();
	}

	function setupDrafts() {
		require(['composer/drafts', 'bootbox'], function (drafts, bootbox) {
			initializeDraftsSidebar(drafts, bootbox);
		});
	}
