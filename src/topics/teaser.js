...
// Pure helper kept at module scope to avoid inflating return count in the export wrapper
...
async function getPreviousNonBlockedPost(postData, blockedUids, deps) {
	const { Topics } = deps;
	let isBlocked = false;
...
async function handleBlocks(uid, teasers, deps) {
	...
	return Promise.all(teasers.map(async (postData) => {
		if (blockedUids.includes(parseInt(postData.uid, 10))) {
			return getPreviousNonBlockedPost(postData, blockedUids, deps);
		}
		return postData;
	}));
}

function createGetTeasers(deps) {
	return async function getTeasers(topics, options) {
		...
		postData = await handleBlocks(uid, postData, deps);
		...
		postData.forEach((post) => {
			...
			if (!Object.prototype.hasOwnProperty.call(users, post.uid)) {
				post.uid = 0;
			}
...
function createGetTeasersByTids(deps) {
	const { Topics } = deps;
	return async function getTeasersByTids(tids, uid) {
...
function createGetTeaser() {
	return async function getTeaser(tid, uid) {
		const teasers = await this.getTeasersByTids([tid], uid);
		return Array.isArray(teasers) && teasers.length ? teasers[0] : null;
	};
}

function createUpdateTeaser(deps) {
	const { Topics } = deps;
	return async function updateTeaser(tid) {
...
module.exports = function (Topics) {
	const deps = { Topics };
	// Bind factories to Topics to avoid returns in this wrapper function
	Topics.getTeasers = createGetTeasers(deps);
	Topics.getTeasersByTids = createGetTeasersByTids(deps);
	Topics.getTeaser = createGetTeaser();
	Topics.updateTeaser = createUpdateTeaser(deps);
};
