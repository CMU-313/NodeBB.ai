...
// 1. Define the logic outside of the export block
async function setCategoryWatchState(uid, cids, state) {
...
async function getCategoryWatchState(uid) {
...
async function getCategoriesByStates(uid, states) {
...
async function ignoreCategory(User, uid, cid) {
	await User.setCategoryWatchState(uid, cid, categories.watchStates.ignoring);
}

async function watchCategory(User, uid, cid) {
	await User.setCategoryWatchState(uid, cid, categories.watchStates.watching);
}

// 2. Keep the export block incredibly simple
module.exports = function (User) {
	User.setCategoryWatchState = (uid, cids, state) => setCategoryWatchState(uid, cids, state);
	User.getCategoryWatchState = (uid) => getCategoryWatchState(uid);
	User.getIgnoredCategories = (uid) => getIgnoredCategories(User, uid);
	User.getWatchedCategories = (uid) => getWatchedCategories(User, uid);
	User.getCategoriesByStates = (uid, states) => getCategoriesByStates(uid, states);
	User.ignoreCategory = (uid, cid) => ignoreCategory(User, uid, cid);
	User.watchCategory = (uid, cid) => watchCategory(User, uid, cid);
};