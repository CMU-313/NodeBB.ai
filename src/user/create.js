...
function buildBulkAddSets(userData, timestamp) {
	const bulkAdd = [
...
async function storePassword(User, uid, password) {
	if (!password) {
		return;
	}
...
async function handlePostCreateActions(User, options) {
	const { userData, data, isFirstUser, userNameChanged } = options;
...
async function create(User, data) {
	const timestamp = data.timestamp || Date.now();
	let userData = buildUserData(data);
	...
	const bulkAdd = buildBulkAddSets(userData, timestamp);
	...
	await Promise.all([
		...
		storePassword(User, userData.uid, data.password),
		User.updateDigestSetting(userData.uid, meta.config.dailyDigestFreq),
	]);

	await handlePostCreateActions(User, { userData, data, isFirstUser, userNameChanged });
	return userData.uid;
}

async function isDataValid(User, userData) {
...
function isPasswordValid(password, minStrength) {
...
async function uniqueUsername(userData) {
	let numTries = 0;
...
module.exports = function (User) {
	...
	User.isDataValid = async function (userData) {
		return await isDataValid(User, userData);
	};

	User.isPasswordValid = isPasswordValid;
